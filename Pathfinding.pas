...
uses
  System.Classes, System.Generics.Collections,uGameGrid, System.Types;
...
begin
  ...
  begin
    ...
    if IsWalkable(NeighborX, NeighborY) then
    begin
...
begin
  Result := (X >= 0) and (Y >= 0) and (X < FCols) and (Y < FRows) and
            (FGameGrid.GetTileType(X, Y) <> 'Wall');
end;

function TPathfinding.FindPath(StartX, StartY, EndX, EndY: Integer): TList<TPoint>;
var
  OpenList, ClosedList: TList<TPathNode>;
  CurrentNode, Neighbor: TPathNode;
  Path: TList<TPoint>;
    ...
    begin
      ...
      for CurrentNode in OpenList do
        if (CurrentNode.FCost < LowestFNode.FCost) or
           ((CurrentNode.FCost = LowestFNode.FCost) and (CurrentNode.HCost < LowestFNode.HCost)) then
