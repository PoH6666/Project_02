...
uses
  ...
  FMX.Types, FMX.Controls, FMX.Forms, FMX.Graphics, FMX.Dialogs, uGameGrid, Pathfinding,
  FMX.Objects, FMX.Layouts, FMX.Controls.Presentation, FMX.StdCtrls, System.Generics.Collections;  // Include the unit for TGameGrid
...
begin
  ...
  begin
    if (MyCharacter.X = DoorPositions[i].X) and (MyCharacter.Y = DoorPositions[i].Y) then
    begin
      if not IsClassRoomOpen then
      begin
        if not Assigned(ClassRoom) then
...
begin
  ...
  with TPathfinding.create(gamegrid, GameGrid.Rows, GameGrid.Cols) do try
    lResult := FindPath( MyCharacter.X,
                          MyCharacter.Y,
                          TargetX,
                          TargetY);

    if assigned(lResult) then begin
...
var
  ...
  Point: TPoint;
begin
  if not Assigned(Pathfinding) then
    Exit;
